...
describe("Teams routes", () => {
  ...
  it("Should return an array of teams with 200 status code", async () => {
    ...
    await db.pool.query(
      "INSERT INTO teams (name, description, created_at, updated_at) VALUES (?, ?, ?, ?)",
      ["Team2", "description2", "2019-03-10 02:55:05", "2019-06-10 00:55:05"]
    );
  ...
  it("Should return team object with 200 status code, [GET:200]", async () => {
    await db.pool.query(
      "INSERT INTO teams (name, description, created_at, updated_at) VALUES (?, ?, ?, ?)",
      ["Team3", "description1", "2019-03-10 02:55:05", "2019-06-10 00:55:05"]
    );
    let team: [Team] = await db.pool.query(
      "SELECT * FROM teams where name ='Team3'"
    );
  ...
  it("Should update team and return 200 status code", async () => {
    await db.pool.query(
      "INSERT INTO teams (name, description, created_at, updated_at) VALUES (?, ?, ?, ?)",
      ["Team4", "description1", "2019-03-10 02:55:05", "2019-06-10 00:55:05"]
    );
    let team: [Team] = await db.pool.query(
      "SELECT * FROM teams where name ='Team4'"
    );
    let res = await chai
      .request(router)
      .put("/teams/" + team[0].id)
  ...
  it("Should delete one or many teams by id and return 204 status code", async () => {
    await db.pool.query(
      "INSERT INTO teams (name, description, created_at, updated_at) VALUES (?, ?, ?, ?)",
      ["TeamA", "description A", "2019-03-10 02:55:05", "2019-06-10 00:55:05"]
    );
    await db.pool.query(
      "INSERT INTO teams (name, description, created_at, updated_at) VALUES (?, ?, ?, ?)",
      ["TeamB", "description B", "2019-03-10 02:55:05", "2019-06-10 00:55:05"]
    );

    let teamA: [Team] = await db.pool.query(
      "SELECT * FROM teams where name ='TeamA'"
    );
    let teamB: [Team] = await db.pool.query(
      "SELECT * FROM teams where name ='TeamB'"
    );

    let teamsIds: String = '"' + teamA[0].id + "," + teamB[0].id + '"';
    let res = await chai
      .request(router)
      .delete("/teams?ids=" + teamsIds)
      .send();
    res.should.have.status(200);
