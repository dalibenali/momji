...
describe("Employees routes", ()=> {
  ...
  it("Should return 404 status code if employee not found", async ()=>  {
    // create new team
    await db.pool.query("INSERT INTO teams (name, description, created_at, updated_at) VALUES (?, ?, ?, ?)", ["TeamLL", "description A", "2019-03-10 02:55:05", "2019-06-10 00:55:05"]);
    let team: [Team] = await db.pool.query("SELECT * FROM teams where name ='TeamLL'");
    ...
    // call employees endpoint with get method
    let res = await chai.request(router).get("/employees/"+notFoundEmployeeId).send();
  ...
  it("Should not update employee if not exist and return 404 status code", async () => {
    ...
    let res = await chai.request(router).put("/employees/"+notFoundEmployeeId).send(newEmployee);
    res.should.have.status(404);
    res.body.should.equal("employee not found");
  });
